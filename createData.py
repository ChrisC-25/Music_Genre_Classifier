...
x = 'filename spectral_centroid chroma_stft spectral_bandwidth rolloff tempo rms spectral_contrast spectral_flatness tonnetz'
for i in range(1, 21):
    x += f' mfcc{i}'
x += ' label'
header = x.split()
...
#this function opens a .csv file and begins writing the data for each .mp3 file in it
    ...
    for track_id in tid:
        ...
        try:
            ...
            to_write = f'{filename} {np.mean(spec_cent)} {np.mean(chroma_stft)} {np.mean(spec_bw)} {np.mean(rolloff)} {np.mean(tempo)} {np.mean(rms)} {np.mean(spec_cont)} {np.mean(spec_flat)} {np.mean(tonnetz)} '    
            for i in mfcc:
